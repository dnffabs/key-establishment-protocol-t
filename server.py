...
def handle_msg(new_socket):
    ...
    print(message)
    # # 发送给所有在线客户端
    # print(socket2user[new_socket])
    # print(type(socket2user[new_socket]))
    for socket in online_socket:
        if socket2user[socket] == username:
...
if __name__ == "__main__":
    try:
        ...
        if available_port:
            print("可用端口号:", available_port)
        else:
            print("未找到可用端口号")

        main_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        main_socket.bind(('127.0.0.1', 49152))  # 服务器绑定的ip和port
